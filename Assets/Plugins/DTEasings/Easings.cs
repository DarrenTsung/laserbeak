...
namespace DTEasings {
	public static class Easings {
		...
		/// Constant Pi.
		/// </summary>
		private const float kPi = Mathf.PI;

		/// <summary>
		/// Constant Pi / 2.
		/// </summary>
		private const float kHalfPi = Mathf.PI / 2.0f;

		public static float Interpolate(float p, EaseType easeType) {
			switch (easeType) {
				default:
		...
		public static float SineEaseIn(float p) {
			return Mathf.Sin((p - 1) * kHalfPi) + 1;
		}
		...
		public static float ElasticEaseIn(float p) {
			return Mathf.Sin(13 * kHalfPi * p) * Mathf.Pow(2, 10 * (p - 1));
		}
		...
		public static float ElasticEaseOut(float p) {
			return Mathf.Sin(-13 * kHalfPi * (p + 1)) * Mathf.Pow(2, -10 * p) + 1;
		}
		...
		public static float ElasticEaseInOut(float p) {
			if (p < 0.5f) {
				return 0.5f * Mathf.Sin(13 * kHalfPi * (2 * p)) * Mathf.Pow(2, 10 * ((2 * p) - 1));
			} else {
				return 0.5f * (Mathf.Sin(-13 * kHalfPi * ((2 * p - 1) + 1)) * Mathf.Pow(2, -10 * (2 * p - 1)) + 2);
			}
		}
		...
		public static float BackEaseIn(float p) {
			return p * p * p - p * Mathf.Sin(p * kPi);
		}
		...
		public static float BackEaseOut(float p) {
			float f = (1 - p);
			return 1 - (f * f * f - f * Mathf.Sin(f * kPi));
		}
		...
		public static float BackEaseInOut(float p) {
			if (p < 0.5f) {
				float f = 2 * p;
				return 0.5f * (f * f * f - f * Mathf.Sin(f * kPi));
			} else {
				float f = (1 - (2 * p - 1));
				return 0.5f * (1 - (f * f * f - f * Mathf.Sin(f * kPi))) + 0.5f;
			}
		}
