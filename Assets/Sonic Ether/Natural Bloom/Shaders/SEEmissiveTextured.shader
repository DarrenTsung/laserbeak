Shader "Sonic Ether/Emissive/Textured" {
	Properties {
		...
		_ForwardDirection("Forward Direction", Vector) = (0, 0, 0, 0)
	...
	SubShader {
		...
		CGPROGRAM
		#pragma shader_feature NO_SMEAR_FORWARD
		...
		void vert(inout appdata_full v, out Input o)
		{
			...
			#ifdef NO_SMEAR_FORWARD
			// NOTE (darren): we only want to smear backwards relative to the object for certain things
			float forwardDot = max(0, dot(normalize(_ForwardDirection), normalize(worldOffset)));
			worldOffset = forwardDot * worldOffset;
			#endif

			fixed3 localOffset = worldPos.xyz - _Position.xyz; // -5

			// World offset should only be behind swing
			float dirDot = max(dot(normalize(worldOffset), normalize(-localOffset)), 0);
			fixed3 unitVec = fixed3(1, 1, 1) * _NoiseHeight;
			worldOffset = clamp(worldOffset, unitVec * -1, unitVec);
			worldOffset *= dirDot * lerp(1, 0, step(length(worldOffset), 0));
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			fixed3 emissTex = tex2D(_Illum, IN.uv2_Illum).rgb;
